...
Замер минимального цикла CPU (декодирование `ldi/sbiw/brne/rjmp`, Apple M1): CPython 3.13 — 7,7 МГц
(медленнее реальных 16 МГц), PyPy 3.11 — 294 МГц, Rust 1.98.1 `-O` — 660 МГц. Поэтому ядро написано на Rust;
текущая скорость полного эмулятора измеряется примером `bench`.
...
| Потенциометр | два резистора, линейно по положению движка | `resistanceOhms`, `position`; каждая часть ≥ 1 Ω — **допущение** |
| Фоторезистор | R = R10 · (E / 10 лк)^(−γ) | R10 и γ по умолчанию — условные значения |
| RGB-светодиод, 7-сегментный индикатор | набор LED той же модели | прямые напряжения по умолчанию — условные |
| Пьезоизлучатель | разрыв цепи; частота по переключениям напряжения на выводах | звук PWM (`analogWrite`) не считается |
| Сервопривод | угол линейно по длительности импульса 544–2400 мкс | поворот мгновенный, механика не моделируется |
...
* Ток утечки входов, входное сопротивление АЦП (100 MΩ), защитные диоды, обратный пробой LED, ограничение тока шин
  ...
  в схеме не моделируются. Компоненты вне перечисленных определений дают `UNSUPPORTED_COMPONENT`.
