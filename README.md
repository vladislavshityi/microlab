...
## Возможности (версия 1.0.0)

* IDE в браузере: редактор кода Monaco с диагностикой компилятора, консоль, Serial Monitor, панель «Проблемы», светлая и тёмная темы.
* Редактор схемы: каталог компонентов, провода между выводами, сетка 2,54 мм, отмена и повтор; макетная плата с реальной топологией.
* Валидация схемы (ошибки, предупреждения, сведения) до компиляции.
* Изолированная компиляция arduino-cli и эмулятор ATmega328P с электрической моделью схемы; запуск, пауза, остановка, reset.
* Проекты на сервере: автосохранение, разрешение конфликтов, история версий с просмотром и восстановлением («Проекты» → «История версий…»), примеры проектов.
* Пользователи, роли, группы и приглашения. История изменений — `CHANGELOG.md`.

## Компоненты и область моделирования

| Компонент | Точность | Что моделируется |
|---|---|---|
| Arduino UNO R3 (ATmega328P) | DIGITAL | CPU, GPIO, INPUT_PULLUP, Timer0/1/2 и PWM, USART0, ADC, прерывания; выходы как источник с выходным сопротивлением |
| Макетная плата | CONNECTIVITY | только связность групп контактов и шин питания |
| Резистор | BASIC_ELECTRICAL | номинальное сопротивление; допуск — справочные данные |
| Светодиод, RGB-светодиод, 7-сегментный индикатор | BASIC_ELECTRICAL | кусочно-линейная модель по прямому напряжению, ток и яркость |
| Кнопка | BASIC_ELECTRICAL | идеальный ключ, без дребезга |
| Потенциометр | BASIC_ELECTRICAL | линейный делитель по положению движка |
| Фоторезистор | BEHAVIORAL | степенная зависимость сопротивления от освещённости |
| Пьезоизлучатель | BEHAVIORAL | частота по переключениям напряжения; звук и акустика не моделируются |
| Сервопривод | BEHAVIORAL | угол по длительности импульса; механика и скорость не моделируются |

Ограничения каждого компонента показываются в панели «Свойства» и перечислены в `packages/circuit-schema/definitions/*.json`; подробности модели — в `simulation/README.md`.

> Симуляция является программной моделью электронного устройства и не заменяет измерения на реальном оборудовании.

## Ограничения

* Не SPICE: только DC рабочая точка, без ёмкостей, индуктивностей, переходных процессов и температуры; PWM усредняется по скважности.
* Не реализованы SPI, I²C (TWI), EEPROM, WDT, режимы сна; загрузчик не эмулируется.
* Единственная плата — Arduino UNO R3; сторонние библиотеки, кроме Servo 1.3.0, недоступны.
* Счётчики ограничений (вход, компиляция) хранятся в памяти единственного процесса API.
...
docker-compose.dev.yml    дополнение для разработки: PostgreSQL и dev-шлюз на 127.0.0.1
...
Состав: `web` (nginx: SPA, прокси `/api/` и WebSocket `/api/v1/ws/`) → `api` (FastAPI) → `postgres`, `compiler`, `simulator`. Контейнер `api` при старте применяет миграции (`alembic upgrade head` под advisory lock PostgreSQL, отключается `MICROLAB_MIGRATE_ON_START=0`), затем запускает uvicorn. `compiler` и `simulator` — отдельные песочницы для недоверенного кода со своими лимитами; в один контейнер они не объединяются, чтобы сборка скетча не имела доступа к процессам и файлам сессий симуляции. Опубликован только порт nginx, по умолчанию на `127.0.0.1`; остальные контейнеры находятся во внутренних сетях без выхода в интернет.
...
API и Vite работают на хосте, в Docker — PostgreSQL, воркер компиляции и сервис симуляции. Контейнеры во внутренних сетях недоступны с хоста, поэтому `docker-compose.dev.yml` публикует PostgreSQL на `127.0.0.1:5433` и добавляет один dev-шлюз `dev-gateway` (nginx): `127.0.0.1:8081` → `compiler` и `127.0.0.1:8082` → `simulator` (сами сервисы остаются без выхода в интернет).

```sh
cp .env.example .env
# в .env раскомментировать: COMPOSE_FILE=docker-compose.yml:docker-compose.dev.yml
docker compose up -d --wait postgres dev-gateway
```
...
Расчёт на типичный сервер 4 CPU / 8 ГБ RAM / 20 ГБ диска (образы ≈2 ГБ) — одна учебная группа (~30 одновременных пользователей). Лимиты в `docker-compose.yml`:

| Сервис | CPU / RAM | Параметры |
|---|---|---|
| `simulator` | 3 CPU / 1 ГБ | `SIM_MAX_SESSIONS=24`: сессия с blink ≈0,13 CPU и ≈3 МБ; 25-я одновременная симуляция получает `SIMULATOR_BUSY` |
| `compiler` | 2 CPU / 1 ГБ | `COMPILER_MAX_CONCURRENT=3`, очередь до 20 с (`COMPILER_QUEUE_WAIT_SECONDS`), затем `COMPILER_BUSY`; сборка ≈1–2 с |
| `api` | — / 512 МБ | пул БД 10 + 10 соединений; Argon2 — не более 3 вычислений одновременно |

Нагрузочная проверка (30 пользователей одновременно: вход, компиляция, повторная компиляция, запуск симуляции, 20 с событий): вход p95 ≈1,6 с; первая компиляция p95 ≈12,5 с (очередь к воркеру); повторная компиляция без изменений — из кэша, ≈10 мс; запуск симуляции p95 <0,1 с; 24 симуляции ≈3 CPU, `api` ≈25 % CPU и ≈100 МБ. Для большего числа одновременных симуляций увеличьте `SIM_MAX_SESSIONS` и `cpus` сервиса `simulator` из расчёта ≈0,15 CPU на сессию.
...
Воркер `services/compiler` запускается только в контейнере: версии arduino-cli, базового образа и платформы зафиксированы, контрольные суммы проверяются при сборке образа; профиль `sketch.yaml` фиксирует FQBN и `arduino:avr (1.8.8)` без сторонних библиотек (доступны только библиотеки платформы). Изоляция: пользователь без root, read-only root FS, `cap_drop: ALL`, `no-new-privileges`, лимиты памяти/CPU/процессов, tmpfs с `noexec`, внутренняя сеть без выхода в интернет. Каждая компиляция — новая временная директория, таймаут 60 с (все процессы сборки завершаются), `prlimit` на CPU/память/размер файлов/число процессов, исходник до 256 KiB, вывод до 64 KB, не более `COMPILER_MAX_CONCURRENT` компиляций одновременно (в `docker-compose.yml` — 3, очередь ждёт 20 с, затем `COMPILER_BUSY`). API кэширует результаты компиляции в памяти (LRU, 256 записей; ключ — SHA-256 версии toolchain и исходника): повторный запуск без изменения кода не компилирует скетч заново, одновременные одинаковые запросы компилируются один раз.

Воркер доступен только во внутренней сети: в полном стеке API обращается к нему напрямую, при разработке — через `dev-gateway` (`127.0.0.1:8081`). Каждый ответ содержит заголовок `X-Request-ID`, этот же id пишется в логи.
